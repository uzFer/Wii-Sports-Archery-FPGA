...
(
    // ---------------------------------------------------------
    // 1. Ports - DONT_TOUCH preserved for Simulation/ILA
    // ---------------------------------------------------------
    (* DONT_TOUCH = "yes" *) input wire [SLAVE_REG_WIDTH-1:0] axi_framewriter_x,
    (* DONT_TOUCH = "yes" *) input wire [SLAVE_REG_WIDTH-1:0] axi_framewriter_y,
    (* DONT_TOUCH = "yes" *) input wire [SLAVE_REG_WIDTH-1:0] axi_framewriter_myScore, 
    ...
    // ---------------------------------------------------------
    // 2. Control & Debug - MARK_DEBUG for Timing Closure
    // ---------------------------------------------------------
    (* MARK_DEBUG = "yes" *) input wire [3:0] game_state,
    (* MARK_DEBUG = "yes" *) output wire bram_en, 
    (* MARK_DEBUG = "yes" *) output wire bram_rst, 
    (* MARK_DEBUG = "yes" *) output wire bram_clk, 
    (* MARK_DEBUG = "yes" *) input wire axi_awvalid,
    (* MARK_DEBUG = "yes" *) input wire axi_awready,
    (* MARK_DEBUG = "yes" *) input wire axi_wvalid,
    (* MARK_DEBUG = "yes" *) input wire axi_wready,
    (* MARK_DEBUG = "yes" *) input wire axi_aclk,
    (* MARK_DEBUG = "yes" *) input wire axi_aresetn,
    
    (* MARK_DEBUG = "yes" *) output wire dbg_out_pixel_to_write,
    (* MARK_DEBUG = "yes" *) output wire dbg_out_is_crosshair_pixel,
    (* MARK_DEBUG = "yes" *) output wire dbg_out_is_target_pixel,
    (* MARK_DEBUG = "yes" *) output wire dbg_out_is_square_pixel
);

    // FSM and Scan Registers
    (* MARK_DEBUG = "yes" *) reg [1:0] current_state; 
    (* MARK_DEBUG = "yes" *) reg [1:0] next_state; 
    (* MARK_DEBUG = "yes" *) reg [SLAVE_REG_WIDTH-1:0] internal_x;
    (* MARK_DEBUG = "yes" *) reg [SLAVE_REG_WIDTH-1:0] internal_y;
    
    // Latch logic for frame consistency
    (* MARK_DEBUG = "yes" *) reg [3:0] score_latch;
    (* MARK_DEBUG = "yes" *) reg [SLAVE_REG_WIDTH-1:0] x_latch, y_latch;
    ...
    // ---------------------------------------------------------
    // 3. Math Logic - MARK_DEBUG (Allows Vivado to use DSPs)
    // ---------------------------------------------------------
    (* MARK_DEBUG = "yes" *) wire [16:0] pixel_index = (internal_y * SCREEN_WIDTH) + internal_x;
    (* MARK_DEBUG = "yes" *) wire [14:0] bram_addr_internal = (pixel_index >> 5) << 2;
    (* MARK_DEBUG = "yes" *) wire [4:0]  pixel_offset = pixel_index % BRAM_DATA_WIDTH;
    (* MARK_DEBUG = "yes" *) wire [31:0] bit_mask = (32'h1 << pixel_offset);

    // Shape Equations
    (* MARK_DEBUG = "yes" *) wire is_square   = (internal_x >= 110 && internal_x <= 210) && (internal_y >= 70 && internal_y <= 170);
    (* MARK_DEBUG = "yes" *) wire is_triangle = (internal_y >= 70 && internal_y <= 170) && 
                                                (internal_x >= (160 - (internal_y - 70))) && (internal_x <= (160 + (internal_y - 70)));
    
    (* MARK_DEBUG = "yes" *) wire signed [15:0] dx = internal_x - 160;
    (* MARK_DEBUG = "yes" *) wire signed [15:0] dy = internal_y - 120;
    (* MARK_DEBUG = "yes" *) wire [31:0] dist_sq = (dx * dx) + (dy * dy);
    (* MARK_DEBUG = "yes" *) wire is_target   = (dist_sq <= 10000 && dist_sq > 6400) || (dist_sq <= 3600 && dist_sq > 1600) || (dist_sq <= 400 && dist_sq > 4);
    (* MARK_DEBUG = "yes" *) wire is_cross    = (internal_x == x_latch) && (internal_y == y_latch);
    ...
    assign bram_clk          = 1'b0; // Driven by Clock Wizard in Block Design
    assign bram_rst          = !axi_aresetn;
