...
`timescale 1 ns / 1 ps
	...
	(
        ...
        // Archery FSM
        (* DONT_TOUCH = "yes" *) input wire [3:0] game_state,  // 3 bit game state
        ...
        (* DONT_TOUCH = "yes" *) output wire bram_en, 
        (* DONT_TOUCH = "yes" *) output wire bram_rst, 
        (* DONT_TOUCH = "yes" *) output wire bram_clk, 

        // AXI lite signals
        (* DONT_TOUCH = "yes" *) input wire axi_awvalid,
        (* DONT_TOUCH = "yes" *) input wire axi_awready,
        (* DONT_TOUCH = "yes" *) input wire axi_wvalid,
        (* DONT_TOUCH = "yes" *) input wire axi_wready,

        // clock and reset
        (* DONT_TOUCH = "yes" *) input wire axi_aclk,
        (* DONT_TOUCH = "yes" *) input wire axi_aresetn,

        // prevent optimization
        (* DONT_TOUCH = "yes" *) output wire dbg_out_pixel_to_write,
        (* DONT_TOUCH = "yes" *) output wire dbg_out_is_crosshair_pixel,
        (* DONT_TOUCH = "yes" *) output wire dbg_out_is_target_pixel,
        (* DONT_TOUCH = "yes" *) output wire dbg_out_is_square_pixel
		// User ports ends
	);

    // define regs/wires for FSM
    (* DONT_TOUCH = "yes" *) reg reg_bram_en;
    (* DONT_TOUCH = "yes" *) reg [BRAM_WE_WIDTH-1:0] reg_bram_we;
    (* DONT_TOUCH = "yes" *) reg [BRAM_ADDR_WIDTH-1:0] reg_bram_addr;
    (* DONT_TOUCH = "yes" *) reg [BRAM_DATA_WIDTH-1:0] reg_bram_wdata;

    // define states
    localparam IDLE  = 2'b00, READ  = 2'b01, WAIT  = 2'b10, WRITE = 2'b11;
    (* DONT_TOUCH = "yes" *) reg [1:0] current_state; 
    (* DONT_TOUCH = "yes" *) reg [1:0] next_state; 

    (* DONT_TOUCH = "yes" *) wire start_rmw_pulse;
    assign start_rmw_pulse = (axi_awvalid && axi_awready && axi_wvalid && axi_wready);
    
    (* DONT_TOUCH = "yes" *) reg [15:0] x_old, y_old;
    (* DONT_TOUCH = "yes" *) reg [3:0] score_old;
    (* DONT_TOUCH = "yes" *) reg frame_busy;
    
    (* DONT_TOUCH = "yes" *) wire update_req = (x_old != axi_framewriter_x) || (y_old != axi_framewriter_y) || (score_old != axi_framewriter_myScore);
    ...
    always @(posedge axi_aclk) begin
        if (!axi_aresetn) begin
            ...
            if (update_req && !frame_busy) begin // only change if one frame is done
                frame_busy <= 1; 
                x_old <= axi_framewriter_x;
                y_old <= axi_framewriter_y;
                score_old <= axi_framewriter_myScore;
    ...
    // Internal pixel counters
    (* DONT_TOUCH = "yes" *) reg [SLAVE_REG_WIDTH-1:0] internal_x;
    (* DONT_TOUCH = "yes" *) reg [SLAVE_REG_WIDTH-1:0] internal_y;
    ...
    // PIXEL TO ADDRESS CALCULATION
    (* DONT_TOUCH = "yes" *) wire [16:0] pixel_index;
    (* DONT_TOUCH = "yes" *) wire [12:0] word_index;
    (* DONT_TOUCH = "yes" *) wire [4:0]  bram_offset;
    (* DONT_TOUCH = "yes" *) wire [14:0] bram_address_calc;
    ...
    (* DONT_TOUCH = "yes" *) wire [31:0] bit_mask;      
    (* DONT_TOUCH = "yes" *) wire [31:0] modified_data; 
    assign bit_mask = (32'h1 << bram_offset);

    // 1. Square Logic (MAIN_MENU)
    (* DONT_TOUCH = "yes" *) wire is_square_pixel = (internal_x >= 110 && internal_x <= 210) && 
                           (internal_y >= 70  && internal_y <= 170);

    // 2. Triangle Logic (GAME_OVER)
    (* DONT_TOUCH = "yes" *) wire is_triangle_pixel = (internal_y >= 70 && internal_y <= 170) &&
                             (internal_x >= (160 - (internal_y - 70))) &&
                             (internal_x <= (160 + (internal_y - 70)));

    // 3. Target Logic (AIM and CALC states)
    (* DONT_TOUCH = "yes" *) wire signed [15:0] dx = internal_x - 160;
    (* DONT_TOUCH = "yes" *) wire signed [15:0] dy = internal_y - 120;
    (* DONT_TOUCH = "yes" *) wire [31:0] dist_sq = (dx * dx) + (dy * dy);
    (* DONT_TOUCH = "yes" *) wire is_target_pixel = (dist_sq <= 10000 && dist_sq > 6400) || 
                           (dist_sq <= 3600  && dist_sq > 1600) || 
                           (dist_sq <= 400   && dist_sq > 4);

    // 4. Crosshair Logic (CALC states)
    (* DONT_TOUCH = "yes" *) wire is_crosshair_pixel = (internal_x == axi_framewriter_x) &&
                              (internal_y == axi_framewriter_y);
                              
    (* DONT_TOUCH = "yes" *) reg pixel_to_write;

    always @(*) begin
//        case (axi_framewriter_myScore) 
          case (score_old) // latched score 
            4'd1: pixel_to_write = is_triangle_pixel; 
            4'd2, 4'd6: pixel_to_write = is_target_pixel;
            4'd4, 4'd8: pixel_to_write = is_crosshair_pixel;      
            4'd12: pixel_to_write = is_square_pixel;
            default: pixel_to_write = 1'b0; 
        endcase
    end

    assign modified_data = (pixel_to_write) ? (bram_read_data | bit_mask) : (bram_read_data & ~bit_mask);
                           
    // prevent optimization assignments                     
    assign dbg_out_pixel_to_write = pixel_to_write; 
    assign dbg_out_is_crosshair_pixel = is_crosshair_pixel;
    assign dbg_out_is_target_pixel = is_target_pixel;
    assign dbg_out_is_square_pixel = is_square_pixel;    
    ...
    always @(*) begin
        ...
        case (current_state) 
            IDLE:  if (frame_busy) next_state = READ;
