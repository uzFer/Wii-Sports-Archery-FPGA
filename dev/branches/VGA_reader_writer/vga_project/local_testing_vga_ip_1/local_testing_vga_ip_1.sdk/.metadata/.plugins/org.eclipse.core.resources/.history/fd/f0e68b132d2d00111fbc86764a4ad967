...
#define PS2_FONT_ROM_INPUT_ADDR XPAR_AXI_GPIO_3_BASEADDR // ascii_in (?)
#define PS2_CONFIG_ADDR_USER_EN XPAR_AXI_GPIO_3_BASEADDR + 0x08 // get_user_input (?)
...
//};
uint32_t coords[3][2] = { {160,25}, {160,120}, {160,185}};
int coord_idx = 0;

uint32_t btn_data;
uint32_t last_btn_data = 0;
volatile uint32_t x_coord =0;
volatile uint32_t y_coord = 0;
...
void draw_crosshair(uint32_t x, uint32_t y) {
   ...
   FRAMEWRITER_mWriteReg(FRAMEWRITER_BASE, REG_Y_OFFSET, (uint32_t)y);
   return;
}


void poll_gyroscope() {
    volatile bool last_valid_bit = false;
