...
import design_4_axi_vip_1_0_pkg::*;
...
module axi_lite_stimulus();
  ...
  * for more details.
  *************************************************************************************************/
  design_4_axi_vip_1_0_mst_t                               agent;
...
localparam FW_BASE_ADDRESS = 'h44A0000;
localparam REG_X_OFFSET = 'h0;
localparam REG_Y_OFFSET = 'h4;
localparam REG_STATE_OFFSET = 'h8;
  initial begin
    /***********************************************************************************************
...
//    agent.start_master();               // agent start to run
    ...
    agent = new("master vip agent",DUT.design_4_i.axi_vip_1.inst.IF);
    agent.start_master();               // agent start to run

    // state transition 
    /*
        main menu (1) -> target (2) -> crosshair only (4) -> game over (12)
        testpoints 
    */
    // --- Frame 1: Main Menu (State 1) ---
    writeRegister(FW_BASE_ADDRESS + REG_STATE_OFFSET, 16'd1);
    #3100us; // Wait slightly longer than 3.072ms for the frame to finish
    
    // --- Frame 2: Target (State 2) ---
    writeRegister(FW_BASE_ADDRESS + REG_STATE_OFFSET, 16'd2);
    #3100us; 
    
    // --- Frame 3: Crosshair at (10, 12) (State 4) ---
    writeRegister(FW_BASE_ADDRESS + REG_X_OFFSET, 16'd10);
    writeRegister(FW_BASE_ADDRESS + REG_Y_OFFSET, 16'd12);
    writeRegister(FW_BASE_ADDRESS + REG_STATE_OFFSET, 16'd4);
    #3100us; 
    
    // --- Frame 4: Game Over (State 12) ---
    writeRegister(FW_BASE_ADDRESS + REG_STATE_OFFSET, 16'd12);
    #3100us;
    
    // Final wait for all drivers to be idle
...
design_4_wrapper DUT (.clk_100MHz(aclk), .reset_rtl_0(aresetn));
