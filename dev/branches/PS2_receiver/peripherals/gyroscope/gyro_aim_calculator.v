...
module gyro_aim_calculator #(
    ...
    parameter CAL_SAMPLES_LOG2 = 5, // 2^5 = 32 samples
    parameter NUMERATOR = 35,       // Scaling numerator (helps approx 8.75 factor)
    ...
    always @(posedge clk) begin
        if (!rst_n) begin
            ...
            case (state)
                ...
                STATE_RUNNING: begin
                    if (calibrate) begin
                        state <= STATE_IDLE; // Re-calibrate
                    end else if (data_valid) begin
                        // Integrate: Acc = Acc + (Rate - Bias)
                        x_acc <= x_acc + (s_x_rate - x_bias);
                        y_acc <= y_acc + (s_y_rate - y_bias);
                        z_acc <= z_acc + (s_z_rate - z_bias);
                    end
                end
